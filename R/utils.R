...
# Function to get the hierarchical prediction sets for the observation in p_test
...
.resampleTwo <- function(p_cal, p_test, y_cal, labels) {
  ...
  for (i in labels) {
    ...
    if (!is.na(des_freq1[i])) {
      idx_category1 <- sample(category, size = des_freq1[i], replace = TRUE)
      idx1 <- c(idx1, idx_category1)
    }
    if (!is.na(des_freq2[i])) {
      idx_category2 <- sample(category, size = des_freq2[i], replace = TRUE)
