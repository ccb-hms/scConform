...
see for example section 1 of @angelopoulos2022gentle 
The output is a 
...
(angelopoulos2025conformalriskcontrol). It considers
...
$P(Y_{new} \in C(X_{new})) \geq 1-\alpha$. More details are 
available at @corbetta2025conformal.
...
percentage. For example, if we set `alpha=0.1`, it means 
...
that the constraint on the miscoverage is satisfied by computing the 
...
than $\lambda$. To choose $\lambda$, we follow eq. (4) in @angelopoulos2025conformalriskcontrol, considering the miscoverage as loss function.
See @corbetta2025conformal for more details 
...
```{r common-ancestor}
# Precompute ontology information to speed up the computation
opi2_cache <- precomputeOnto(opi2)
# Add a new variable to the colData with the common ancestor for each prediction set
spe_query$pred_set_hier_simp <- vapply(
    spe_query$pred_set_hier,
    function(x) getCommonAncestor(x, opi2, opi2_cache),
    character(1)
)
