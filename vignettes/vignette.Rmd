...
1. The first method uses split conformal inference (ref?). The output is a 
...
2. The second method is based on conformal risk control (ref). It considers
...
given a directed graph let $\mathcal{P}(v)$ and $\mathcal{A}(v)$ be the set
...
$$
\mathcal{P}(v) \cup \{\mathcal{P}(a): a\in\mathcal{A}(\hat{y}(x)): 
g(a,x)\leq\lambda \},
$$
...
than $\lambda$. To choose $\lambda$, we follow eq. (4) in Anastasios et al. 
