...
```{r predsets}
labels <- colnames(p.test)
sets <- getPredictionSets(
    x.query = p.test, 
    x.cal = p.cal, 
    y.cal = spe.cal$cell_type,
    alpha = 0.1,
...
head(colData(spe.query))
head(spe.query$pred_set)
```
...
`follow.ontology=TRUE` and provide an ontology to the `onto` parameter.
...
head(spe.query$pred_set_hier_simp)
```
