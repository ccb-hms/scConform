...
As an example, we'll use the mouse ileum Merfish data from the `MerfishData` Bioconductor package, segmented with Baysor.
...
```{r data}
# Load data
spe_baysor <- MouseIleumPetukhov2021(segmentation = "baysor", 
...
```{r}
set.seed(1635)
ref <- sample(1:ncol(spe_baysor), 1500)
...
set.seed(1703)
train <- sample(1:ncol(spe_ref), 500)
# Training data
...
As an alternative, we can choose to return, only the common ancestor, instead of 
the entire set of predicted labels. This can be done directly in the function by 
setting `simplify_pred=TRUE`.
Alternatively, we can convert the obtained prediction sets invoking
the function `returnCommonAncestor`.

```{r common-ancestor}
spe_query$pred_set_hier_simp <- sapply(spe_query$pred_set_hier,
...
```{r}
plotResult(spe_query$pred_set[[64]], opi2, 
           col_grad="pink", attrs=attrs, add_scores = FALSE,
           title="Conformal Prediction set")
```

```{r}
plotResult(spe_query$pred_set_hier[[64]], opi2,
           col_grad="pink", attrs=attrs, add_scores = FALSE,
...
```{r}
plotResult(spe_query$pred_set[[64]], opi2, probs=p_test[64,],
           col_grad=c("lemonchiffon", "orange", "darkred"), 
           attrs=attrs, add_scores = FALSE,
           title="Conformal Prediction set")
```
```{r}
plotResult(spe_query$pred_set_hier[[64]], opi2, probs=p_test[64,],
           col_grad=c("lemonchiffon", "orange", "darkred"), 
           attrs=attrs, add_scores = FALSE,
           title="Hierarchical Prediction set")
```
...
plotResult(spe_query$pred_set[[64]], opi2, probs=p_test[64,],
           col_grad=c("lemonchiffon", "orange", "darkred"), 
           attrs=attrs, add_scores = TRUE, k=2,
           title="Conformal Prediction set")
```
```{r}
plotResult(spe_query$pred_set_hier[[64]], opi2, probs=p_test[64,],
